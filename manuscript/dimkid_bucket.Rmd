...
```{r, include = T, fig.width = 7, fig.asp = 0.45}
d2_all_endorse %>%
  filter(factor %in% c("BODY", "HEART")) %>%
  select(age_group, subid, character, factor, sum_endorse) %>%
  spread(factor, sum_endorse) %>%
  ggplot(aes(x = BODY, y = HEART, color = character)) +
  facet_grid(~ age_group) +
  geom_abline(lty = 2) + 
  annotate("rect", xmin = 3.5, xmax = Inf, ymin = -Inf, ymax = 2.5, 
           fill = "lightblue", alpha = 0.25) +
  annotate("rect", xmin = -Inf, xmax = 2.5, ymin = 3.5, ymax = Inf, 
           fill = "lightblue", alpha = 0.25) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0, size = 3) +
  annotate("text", x = 6.5, y = -0.5, hjust = 1, label = "BODY without HEART") +
  annotate("text", x = -0.5, y = 6.5, hjust = 0, label = "HEART without BODY") +
  scale_color_brewer(palette = "Paired") +
  scale_x_continuous("\nBODY: # items endorsed", breaks = seq(0, 6, 1)) +
  scale_y_continuous("HEART: # items endorsed\n", breaks = seq(0, 6, 1)) +
...
d2_all_endorse %>%
  filter(factor %in% c("BODY", "MIND")) %>%
  select(age_group, subid, character, factor, sum_endorse) %>%
  spread(factor, sum_endorse) %>%
  ggplot(aes(x = BODY, y = MIND, color = character)) +
  facet_grid(~ age_group) +
  geom_abline(lty = 2) + 
  annotate("rect", xmin = 3.5, xmax = Inf, ymin = -Inf, ymax = 2.5, 
           fill = "lightblue", alpha = 0.25) +
  annotate("rect", xmin = -Inf, xmax = 2.5, ymin = 3.5, ymax = Inf, 
           fill = "lightblue", alpha = 0.25) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0, size = 3) +
  annotate("text", x = 6.5, y = -0.5, hjust = 1, label = "BODY without MIND") +
  annotate("text", x = -0.5, y = 6.5, hjust = 0, label = "MIND without BODY") +
  scale_color_brewer(palette = "Paired") +
  scale_x_continuous("\nBODY: # items endorsed", breaks = seq(0, 6, 1)) +
  scale_y_continuous("MIND: # items endorsed\n", breaks = seq(0, 6, 1)) +
  theme(panel.grid.minor = element_blank(),
        text = element_text(size = 22),
        # panel.grid.major = element_line(color = "gray50"),
        legend.position = "top") +
...
d2_all_endorse %>%
  filter(factor %in% c("HEART", "MIND")) %>%
  select(age_group, subid, character, factor, sum_endorse) %>%
  spread(factor, sum_endorse) %>%
  ggplot(aes(x = HEART, y = MIND, color = character)) +
  facet_grid(~ age_group) +
  geom_abline(lty = 2) + 
  annotate("rect", xmin = 3.5, xmax = Inf, ymin = -Inf, ymax = 2.5, 
           fill = "lightblue", alpha = 0.25) +
  annotate("rect", xmin = -Inf, xmax = 2.5, ymin = 3.5, ymax = Inf, 
           fill = "lightblue", alpha = 0.25) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0, size = 3) +
  annotate("text", x = 6.5, y = -0.5, hjust = 1, label = "HEART without MIND") +
  annotate("text", x = -0.5, y = 6.5, hjust = 0, label = "MIND without HEART") +
  scale_color_brewer(palette = "Paired") +
  scale_x_continuous("\nHEART: # items endorsed", breaks = seq(0, 6, 1)) +
  scale_y_continuous("MIND: # items endorsed\n", breaks = seq(0, 6, 1)) +
  theme(panel.grid.minor = element_blank(),
        text = element_text(size = 22),
        # panel.grid.major = element_line(color = "gray50"),
