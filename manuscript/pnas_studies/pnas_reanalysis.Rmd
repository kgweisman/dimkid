...
app_plot_fun_pnas <- function(df, which_efa){
  ...
  plot <- means %>%
    ggplot(aes(x = factor, y = mean, color = condition, fill = condition)) +
    geom_hline(yintercept = 0, lty = 2) +
    geom_point(data = bypart, aes(y = score), alpha = 0.5,
...
```{r}
app_plot_fun_pnas_s3 <- function(df, which_efa){
...
```{r}
app_plot_fun_pnas_s4 <- function(df, which_efa){
...
```{r, include = T}
app_plot_fun_pnas(d1, efa_pnasd1) +
  labs(title = "PNAS Study 1")
```

## Study 2

```{r, include = T}
app_plot_fun_pnas(d2, efa_pnasd2) +
  labs(title = "PNAS Study 2")
```

## Study 2

```{r, include = T}
app_plot_fun_pnas_s3(d3_all, efa_pnasd3) +
  labs(title = "PNAS Study 3")
```

## Study 4

```{r, fig.width = 5, fig.asp = 0.5, include = T}
app_plot_fun_pnas_s4(d4 %>%
                       mutate(condition = factor(condition,
                                                 levels = c("stapler", "car",
                                                            "computer", "robot",
                                                            "microbe", "beetle",
