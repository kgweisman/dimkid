---
title: "Children's developing representations of mental life: Changes in conceptual structure between 4-9 years of age"
...
We dropped trials with response times that were faster than a preset criterion of 250ms (_n_=3 child trials, _n_=97 adult trials) and retained participants regardless of skipped trials (_n_=55 child trials, _n_=1 adult trial). Overall, only 1% of adult trials and 1% of child trials were missing data.  
...
nfact_par_d1_ad <- fa.parallel(d1_ad_wide, cor = chosen_cor, 
                               fm = chosen_fm)$nfact
nfact_bic_d1_ad <- vss(d1_ad_wide, cor = chosen_cor, rotate = chosen_rot, 
                       fm = chosen_fm)$vss.stats$BIC %>% which.min()
nfact_wdm_d1_ad <- reten_fun(d1_ad_wide, rot_type = chosen_rot)
...
```{r}
efa_3_d1_ad <- fa_fun(d1_ad_wide, 3)
efa_3_plot_d1_ad <- heatmap_fun(efa_3_d1_ad, 
                                factor_names = c("HEART", "BODY", "MIND")) +
  labs(title = paste0("Adults (n=", nrow(d1_ad_wide), ")")) +
  guides(fill = "none")
```
...
```{r}
efa_3_d1_79 <- fa_fun(d1_79_wide, 3)

efa_3_plot_d1_79 <- heatmap_fun(efa_3_d1_79, 
                                factor_names = c("HEART", "BODY", "MIND")) +
  labs(title = paste0("Children 7-9y (n=", nrow(d1_79_wide), ")"))
```
...
To formally compare responses from children and adults, we considered the full, combined dataset and examined factor scores by age group. 

```{r}
d1_all_wide <- d1_ad_wide %>%
  rownames_to_column("subid_char") %>%
  full_join(d1_79_wide %>%
              rownames_to_column("subid_char")) %>%
...
```{r}
# implement 3 factor retention protocols
nfact_par_d1_all <- fa.parallel(d1_all_wide, 
                                cor = chosen_cor, fm = chosen_fm)$nfact
nfact_bic_d1_all <- vss(d1_all_wide, cor = chosen_cor, rotate = chosen_rot,
                        fm = chosen_fm)$vss.stats$BIC %>% which.min()
nfact_wdm_d1_all <- reten_fun(d1_all_wide, rot_type = chosen_rot)
...
```{r}
efa_3_d1_all <- fa_fun(d1_all_wide, 3)
efa_3_plot_d1_all <- heatmap_fun(efa_3_d1_all,
                                 factor_names = c("HEART", "BODY", "MIND")) +
  labs(title = "Adults and children, combined")
```

EFA using the combined dataset revealed three factors that met our retention criteria. Unsurprisingly, these three factors were very similar to those revealed for adults and children analyzed independently: They corresponded to social-emotional abilities (_HEART_), physiological sensations (_BODY_), and perceptual-cognitive abilities (_MIND_; see SOM). 

The purpose of this combined EFA was to examine differences in adults' and children's evaluations of beetles and robots within this three-part structure. To do so, we derived factor scores (via the ten Berge method) using the rotated three-factor solution (using medians by capacity to impute missing values). This yielded three scores for each participant, corresponding, in principle, to holistic judgments of the social-emotional, physiological, and perceptual-cognitive abilities of the target character the participant evaluated. (Note that each of these three scores takes into account factor loadings for all 40 mental capacities, as listed in SOM, Table S1.) 
This allowed us to examine the effects of age group (adult, child), character (beetle, robot), and factor (_BODY_, _HEART_, _MIND_) on these scores via mixed effects linear regression. See Table XX for the results of a maximal model and Figure 2 for scores by age group, age (for children), factor, and character. 

```{r}
efa_3_scores_d1_all <- efa_3_d1_all$scores %>%
  data.frame() %>%
  rownames_to_column("subid_char") %>%
  ...
  left_join(full_join(d1_ad %>% distinct(subid, age_group),
                      d1_79 %>% distinct(subid, age_group, age))) %>%
...
```{r}
fig02_plot <- ggplot(efa_3_scores_d1_all %>%
                       filter(!is.na(age)),
                     aes(x = age, y = score,
  ...
  geom_pointrange(data = efa_3_scores_boot_d1_all %>% filter(age_group == "adults"),
                  ...
                  # color = "black", size = 1, fatten = 1.25) +
...
fig02_with_caption <- add_sub(fig02_plot, str_wrap("Figure 2: Factor scores for adults and 7- to 9-year-old children in Study 1, in which each participant assessed 40 mental capacities for one of two target entities: a beetle (red circles) or a robot (turquoise squares). Each participant recieved a factor score for each factor (BODY, HEART, and MIND), which we treated a summary of that participants' mental capacity attributions in that domain; a score of 0 corresponds to the mean for that factor (collapsing across ages). Lines correspond to linear regressions considering children's scores alone. Error bars are bootstrapped 95% confidence intervals on adults' scores. Note that the range of the y-axis differs across panels.", 134), x = 0, hjust = 0)
ggdraw(fig02_with_caption)
```

**BOOKMARK**

Collapsing across age groups and factors, factor scores suggest that participants generally attributed fewer mental capacities to the robot than the beetle (b=XX). However, this appears to be entirely due to the huge discrepancy between characters in the physiological domain; the difference between characters was reduced to nothing in the social-emotional domain (b=XX), and reversed in the perceptual-cognitive domain (b=XX). Collapsing across entities (beetle, robot), children tended to attribute more mental capacities adults (b=XX), but this was driven primarily by the social-emotional domain (b=XX), and was reversed in the perceptual-cognitive domain (b=XX). 

Scores in the physiological and perceptual-cognitive domains were very similar for children and adults: Both children and adults marked a clear difference between the robot and the beetle in the physiological domain (Figure XX, center), in line with the animate–inanimate distinction; and both age groups credited the robot with slightly greater perceptual-cognitive skills than the beetle (right). In contrast, in the social-emotional domain (Figure XX, left) both the beetle and the robot received rather low scores among adults, but very high scores among children. See Figure XX for raw counts of no, kinda, and yes responses for all items, grouped by character, age group, and dominant factor. 
