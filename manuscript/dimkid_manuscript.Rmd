...
Instructions were identical to Study 1. Participants rated the target character on 20 mental capacities, presented in a random order for each participant. On each trial, participants responded _no_, _kinda_, or _yes_ to the question "Do you think a [target] can…?" As in Study 1, adults completed the study by clicking through a website at their own pace, with one trial presented on each page and no ability to go backwards, and children completed the study on an experimenter's laptop computer. For older children the experimenter read the instructions and the first several trials out loud, requesting verbal responses from the child and selected his or her response for her; after several trials, the experimenter gave the child the option to continue independently if they desired. For younger children, the experimenter read all questions out loud and children responded verbally. 
...
#   theme_bw() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
#   labs(x = "mental capacity", y = "proportion of participants")


# fig07_BODY_plot <- d2_all %>%
#   filter(factor == "BODY") %>%
#   mutate(capacity_pad = str_pad(capacity, 
#                                 max(nchar(d2_all$capacity)) + 3, 
#                                 "left")) %>%
...
#   theme_bw() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
#   labs(y = "proportion of participants", x = "mental capacity") +
...
#   theme_bw() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
#   labs(y = "proportion of participants", x = "")
...
fig07_fun <- function(whichfac){
  ...
  fig <- d2_all %>%
    ...
    theme_bw() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
    labs(y = "proportion of participants", x = "mental capacity (by factor)")
