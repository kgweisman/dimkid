...
subtitle: Draft updated 2018-09-25
output:
  html_notebook:
    toc: yes
    toc_depth: 4
...
`r d3_ad %>% distinct(subid) %>% count() %>% as.numeric() + d3_46 %>% distinct(subid) %>% count() %>% as.numeric()` people participated in this study, including a group of adults and a group of 4- to 6-year-old children.
...
efa_4_plot_d3_46 <- heatmap_fun(efa_4_d3_46,
                                factor_names = c("MIND*", "BODY I", 
                                                 "BODY II", "HEART*")) +
  labs(title = paste0("Children (4-6y): 4-factor solution")) +
  guides(fill = "none")
```
...
efa_3_plot_d3_46 <- heatmap_fun(efa_3_d3_46,
                                factor_names = c("BODY", "MIND*", "HEART*")) +
  labs(title = paste0("Children (4-6y): 3-factor solution")) +
  guides(fill = "none")
```
...
The third factor corresponded primarily to social-emotional abilities, as well as one positive emotion (_feel happy_). An analysis of factor congruence confirmed that this factor was most similar to adults' _HEART_ factor (cosine similarity with _MIND_: `r cong_efa_3_d3_ad_46["F3", "F2"]`; with _BODY_: `r cong_efa_3_d3_ad_46["F3", "F1"]`; with _MIND_: `r cong_efa_3_d3_ad_46["F3", "F3"]`). It was the dominant factor for such items as `r top_n_domCap(efa_3_d3_46, 4, "F2", "pos")`, and accounted for `r round(efa_3_d3_46$Vaccounted["Proportion Explained", "F2"], 2) * 100`% of the shared variance in the rotated three-factor solution. (See Figure 9, Panel B, for all factor loadings.)

In relation to the _BODY_-_HEART_-_MIND_ structure found among older children and adults, we would describe this three-factor structure as being fairly similar to the adult-like conceptual structure—with the notable exception that half of the social-emotional abilities (_get sad_, _hate someone_, and _feel happy_) did not load most strongly on children's ostenstive _HEART\*_ factor.
