...
fig08a_plot <- ggplot(d2_anim %>% 
  ...
  scale_color_manual(values = c("firebrick1", "white", 
                                "darkorchid", "slategrey"), 
                     breaks = c("animate", "inanimate", 
                                "dummy", "inan_norobot"),
                     labels = c("animates", "inanimates", 
                                " ", "inanimates (no robot)"),
                     na.value = "gray") +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        legend.position = "bottom") +
  guides(color = guide_legend(override.aes = list(alpha = 1),
                              nrow = 2, byrow = F, reverse = F),
         fill = guide_legend(nrow = 2, byrow = F, reverse = F)) +
...
```{r, include = T, fig.width = 6, fig.asp = 1}
fig08_with_caption <- add_sub(fig08_plots, str_wrap("Figure 8: Factor scores for adults and 4- to 9-year-old children in Study 1, replotted to illustrate age effects within the child samples. We present two ways of looking at these data: collapsing characters into animates (beetle, bird, mouse, goat, elephant) vs. inanimates (computer, robot, doll, teddy bear; Panel A); and examining each character individually (Panel B). In Panel A, we additionally plot age-related changes in attributions to inanimates after excluding participants who assessed the robot in particular (purple liness and points). Lines correspond to linear regressions considering children's scores alone. Error bars are bootstrapped 95% confidence intervals on adults' scores. Note that the range of the y-axis differs across panels.", 138), x = 0, hjust = 0)
