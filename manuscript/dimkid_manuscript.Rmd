...
Two or our three methods for determining how many factors to retain (minimizing BIC and Weisman et al.'s factor retention criteria) suggested retaining three factors. (Parallel analysis suggested retaining four factors. Three of these factors were very similar to the three factors reported below, and the fourth factor was dominant for a handful of sensory and perceptual capacities, including top_n_domCap(efa_4_d1_ad, 4, "F4", "pos"); see SOM.)
...
fixef(efa_3_scores_r_d1_all) %>%
  ...
  group_rows("Older children vs. adults", 7, 12)
```
...
fixef(d2_anim_r) %>%
  ...
  group_rows("Adults", 1, 6) %>%
  group_rows("Older children vs. adults", 7, 12) %>%
  group_rows("Younger children vs. adults", 13, 18)
```

By definition, adults' factor scores did not differ across factors (HEART vs. grand mean: `r write_b_95CI_fun(d2_anim_r, "factor_nameH_GM")`; MIND vs. grand mean: `r write_b_95CI_fun(d2_anim_r, "factor_nameM_GM")`). As we would expect, adults attributed more mental capacities (collapsing across factors) to animates than inanimates (`r write_b_95CI_fun(d2_anim_r, "anim_inananim_GM")`)—a difference that was diminished in the _HEART_ domain (`r write_b_95CI_fun(d2_anim_r, "factor_nameH_GM:anim_inananim_GM")`), but not substantially diminished in the_MIND_ domain (`r write_b_95CI_fun(d2_anim_r, "factor_nameM_GM:anim_inananim_GM")`).
...
```{r}
fig06b_plot <- ggplot(efa_3_scores_d2_all %>%
                        filter(!is.na(age)) %>%
                        mutate(facet_lab = "individual characters"),
                      aes(x = age, y = score,
                          fill = character, color = character)) +
  facet_grid(cols = vars(facet_lab), rows = vars(factor_name), scales = "free") +
  geom_hline(yintercept = 0, lty = 2) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "lm", alpha = 0.10, show.legend = F) +
...
```{r, include = T, fig.width = 6, fig.asp = 1}
fig06_with_caption <- add_sub(fig06_plots, str_wrap("Figure 6: Factor scores for 4- to 9-year-old children and adults in Study 2, in which each participant assessed 20 mental capacities for one of nine target characters. Each participant recieved a factor score for each factor (BODY, HEART, and MIND, as defined by an exploratory factor analysis of adults' responses). We treated these scores as summaries of that participants' mental capacity attributions in these domains; a score of 0 corresponds to the mean for that factor among adults. Lines correspond to linear regressions considering children's scores alone. Error bars are bootstrapped 95% confidence intervals on adults' scores. Note that the range of the y-axis differs across panels.", 138), x = 0, hjust = 0)
ggdraw(fig06_with_caption)
```

```{r}
# d2_79_anim_r <- brm(score ~ factor_name * anim_inan * age +
#                       (1 | subid) +
...
#                     control = list(adapt_delta = 0.99),
#                     cores = 4)
# saveRDS(d2_79_anim_r, file = "./brms_models/d2_79_anim_r.rds")

d2_79_anim_r <- readRDS("./brms_models/d2_79_anim_r.rds")
# summary(d2_anim_r)
```
...
A separate regression on children's factor scores alone confirmed that, while collapsing across factors and target characters children's overall mental capacity attributions did not vary with age (`r write_b_95CI_fun(d2_79_anim_r, "age")`), the distinction between animates and inanimates in terms of attributions of _HEART_ decreased with age (`r write_b_95CI_fun(d2_79_anim_r, "factor_nameH_GM:anim_inananim_GM:age")`). (See Table 4 for the full results of this model, and Figure 6 for a visualization of changes in children's factor scores across age.)
