---
title: "Dimkid Paper #1 Analysis (7-9yo and Adults)"
output:
  html_notebook:
    theme: flatly
    toc: yes
...
```{r s1 demographics by condition}
# gender
with(d1 %>%
...
```{r s1 how many factors part 1, include = F}
...
```{r s1 all rotated, include = F}
efa_d1_all_rotated <- fa(d1_all, 13, rotate = chosenRotType,
                           cor = chosenCorType, fm = "minres")
fa.sort(efa_d1_all_rotated)
```
...
```{r s2 demographics by condition}
# gender
with(d2 %>%
       distinct(subid, character, gender) %>%
       filter(!is.na(gender)) %>%
...
```{r s2 how many factors part 1, include = FALSE}
# examine eigenvalues and variance explained
...
```{r s2 all rotated, include = FALSE}
efa_d2_all_rotated <- fa(d2_all, 13, rotate = chosenRotType,
                           cor = chosenCorType, fm = "minres")
fa.sort(efa_d2_all_rotated)
```
...
```{r s12 figure factor scores by age group, fig.width = 8, fig.height = 3}
# plot
...
```{r s1 factor scores by age setup, fig.width = 8, fig.height = 3}
...
```{r s2 factor scores by age regression}
...
```{r s2 factor scores by age plot, fig.width = 8, fig.height = 3}
...
```{r s12 scatter plus mean scores by age setup, fig.width = 8, fig.height = 3}
tempA <- d2 %>% 
  select(subid, age, character) %>% 
  distinct() %>% 
  mutate(subid = paste(character, subid, sep = "_")) %>%
