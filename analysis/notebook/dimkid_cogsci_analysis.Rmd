...
d_raw_study3 <- read.csv("/Users/kweisman/Documents/Research (Stanford)/Projects/Dimkid/dimkid/data/children/run-02_2017-07-24_anonymized.csv") %>%
...
d_raw_study4 <- read.csv("/Users/kweisman/Documents/Research (Stanford)/Projects/Dimkid/dimkid/data/children/run-03_2017-07-24_anonymized.csv") %>%
...
```{r all studies loadings table, include = F}
...
bigTable <- order_s1 %>%
  ...
  select(order1, capacity, ends_with("MR1"), ends_with("MR2"), ends_with("MR3"))
...
# set up labels for plot (shortened version of mental capacity items)
wording <- loadings(efa_d2_all_rotatedN)[] %>%
  data.frame() %>%
  rownames_to_column(var = "item") %>%
  select(item) %>%
  mutate(wording = factor(
...
```{r figure heatmap s1, fig.width = 7, fig.height = 6}
...
```{r figure heatmap s2, fig.width = 7, fig.height = 6}
...
```{r figure heatmap s3, fig.width = 5, fig.height = 4}
...
ggplot(scores_s3_plotting %>%
         ...
         mutate(character = factor(character, 
                ...
                factor = factor(factor,
                                ...
                                labels = c("Social-emotional", "Physiological", "Perceptual-cognitive"))),
...
```{r s34 factor scores by age setup, fig.width = 6, fig.height = 3}
# scores_s34_plotting <- d3 %>% 
#   select(subid, age, character) %>% 
#   distinct() %>% 
#   mutate(subid = paste(character, subid, sep = "_")) %>%
#   full_join(d4 %>%
#               mutate(age = (as.numeric(dateOfTest) - as.numeric(dateOfBirth))/365) %>%
#               select(subid, age, character) %>%
#               distinct() %>%
#               mutate(subid = paste(character, subid, sep = "_"))) %>%
#   full_join(efa_d3_all_rotatedN$scores %>% 
