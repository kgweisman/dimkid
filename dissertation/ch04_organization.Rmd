...
```{r}
regtab_studies1abc <- regtab_d1a_ad_scored_ad_diff %>%
...
```{r}
regtab_studyd <- regtab_d1d_ad_scored_ad_diff %>%
  mutate_at(vars(b, s.e.),
            funs(format(round(., digits = 2), nsmall = 2))) %>%
  unite(result, b, s.e., CI95, nonzero) %>%
  spread(study, result) %>%
...
regtab_d2_ad79_scored_ad_diff <- diff_reg_table_fun(
  ...
  char_label = c("Robot vs. GM"), agegp_label = c("Children vs. adults"))
```
