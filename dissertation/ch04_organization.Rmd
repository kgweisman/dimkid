...
```{r}
plot_d1d_ad_scored_ad_diff <- diffplot_fun(d1d_ad_scored_ad_diff, colors21)
```
...
figure4.2_plots4 <- plot_grid(
  plot_d1d_ad_scored_ad_diff +
    labs(title = "Study 1d: Adults") +
    theme(legend.position = "bottom") +
    scale_color_manual("Target character", values = c(colors21, "black"),
                       guide = guide_legend(direction = "horizontal", ncol = 7)) +
    scale_fill_manual("Target character", values = c(colors21, "black"),
                      guide = guide_legend(direction = "horizontal", ncol = 7)) +
    scale_size_manual("Target character", values = c(rep(0.5, 21), 0.8),
                      guide = guide_legend(direction = "horizontal", ncol = 7)),
  labels = "D")

figure4.2_plots <- plot_grid(figure4.2_plots123, figure4.2_plots4,
...
figure4.10_cap <- add_sub(figure4.10_plots, str_wrap("Figure 4.10: Difference scores between US adults' and children's attributions of conceptual units in Study 3. this includes difference scores using adults' BODY, HEART, and MIND scales (panel B) and difference scores using children's own scales (panel C; see Table 4.2). For each conceptual unit, scores could range from 0-1, such that difference scores could range from -1 to +1. Individual participants are plotted as small, translucent circles, and mean difference scores by character are plotted as larger, solid diamonds. Error bars are 95% bootstrapped confidence intervals. The dotted line corresponds to equal endorsements of the two conceptual units plotted (i.e., a difference score of 0).", 180), x = 0, hjust = 0)
```
...
figure4.11_plots <- plot_grid(
  plot_d3_79_scored_79_diff +
    labs(title = "Study 3: Children, 7-9y (scored using their own scales)") +
    theme(legend.position = "bottom"),
  plot_d3_46_scored_46_wdm_diff + 
    labs(title = "Study 3: Children, 4-6y (scored using their own scales, 3-factor solution)") +
    theme(legend.position = "bottom"), 
  plot_d3_46_scored_46_par_diff + 
    labs(title = "Study 3: Children, 4-6y (scored using their own scales, 2-factor solution)") +
    theme(legend.position = "bottom"), 
  ncol = 3, rel_widths = c(1, 1, 1),
  labels = "AUTO")

figure4.11_cap <- add_sub(figure4.11_plots, str_wrap("Figure 4.11: Difference scores between 7- to 9-year-old children's and 4- to 6-year-old children's attributions of conceptual units in Study 3, using their own scales. For younger children (4-6y of age), this includes difference scores using the three-factor EFA solution (panel B) and difference scores using the two-factor solution (panel C; see Table 4.2). For each conceptual unit, scores could range from 0-1, such that difference scores could range from -1 to +1. Individual participants are plotted as small, translucent circles, and mean difference scores by character are plotted as larger, solid diamonds. Error bars are 95% bootstrapped confidence intervals. The dotted line corresponds to equal endorsements of the two conceptual units plotted (i.e., a difference score of 0).", 180), x = 0, hjust = 0)
```
