...
figure4.2_cap <- add_sub(figure4.2, str_wrap("Figure 4.2: Relationships among US adults' and 7- to 9-year-old children's attributions of conceptual units in Study 2, organized by sample and scales used (rows) and by pair of conceptual units (columns). For children, two sets of scores are presented: using adults' scales (B1-B3) and using children's own scales (C1-C3; see Table 4.2 for differences in these scales). For each conceptual unit, scores could range from 0-1. Individual participants are plotted as small, translucent circles, and mean scores by character are plotted as larger, solid diamonds. Error bars are 95% bootstrapped confidence intervals. The dotted line corresponds to equal endorsements of the two conceptual units plotted.", 110), x = 0, hjust = 0)
```
...
```{r}
figure4.3 <- plot_grid(figure4.3a_plots_leg_title, figure4.3b_plots_leg_title,
                       figure4.3c_plots_leg_title, figure4.3d_plots_leg_title, 
                       figure4.3e_plots_leg_title, figure4.3f_plots_leg_title,
                       ncol = 1)

figure4.3_cap <- add_sub(figure4.3, str_wrap("Figure 4.3: Relationships among US adults', older children's, and younger children's attributions of conceptual units in Study 3, organized by sample and scales used (rows) and by pair of conceptual units (columns). For older children (7-9y of age), two sets of scores are presented: using adults' scales (B1-B3) and using older children's own scales (C1-C3). For younger children (4-6y of age), three sets of scores are presented: using adults' scales (D1-D3), using younger children's own scales as indexed by the three-factor EFA solution (E1-E3), and using younger children's own scales as indexed by the two-factor EFA solution (F1). See Table 4.3 for all scales. For each conceptual unit, scores could range from 0-1. Individual participants are plotted as small, translucent circles, and mean scores by character are plotted as larger, solid diamonds. Error bars are 95% bootstrapped confidence intervals. The dotted line corresponds to equal endorsements of the two conceptual units plotted.", 110), x = 0, hjust = 0)
```

```{r, include = T, fig.width = 5, fig.asp = 2.65}
...
```{r}
figure4.4a_plots <- plot_grid(plots_d4_ad_scored_ad[[1]] + 
                               theme(legend.position = "none"),
...
figure4.4a_leg <- get_legend(plots_d4_ad_scored_ad[[1]] +
                               theme(legend.position = "bottom", 
                                     legend.direction = "horizontal") +
                               scale_fill_manual("Target character", 
                                                 values = colors02,
...
figure4.4a_plots_leg <- plot_grid(figure4.4a_plots,
                                  figure4.4a_leg,
                                  ncol = 1, rel_heights = c(1, 0.05))

figure4.4a_title <- ggdraw() + 
  draw_label("Study 4: Adults", size = 16, fontface = 'bold', x = 0, hjust = 0)

figure4.4a_plots_leg_title <- plot_grid(figure4.4a_title, figure4.4a_plots_leg,
                                        ncol = 1, rel_heights = c(0.12, 1))
```

```{r, fig.width = 6, fig.asp = 0.4}
figure4.4a_plots_leg_title
```
...
```{r}
figure4.4b_plots <- plot_grid(plots_d4_46_scored_ad[[1]] + 
                               theme(legend.position = "none"),
...
figure4.4b_leg <- get_legend(plots_d4_46_scored_ad[[1]] +
                               theme(legend.position = "bottom", 
                                     legend.direction = "horizontal") +
                               scale_fill_manual("Target character", 
                                                 values = colors02,
...
figure4.4b_plots_leg <- plot_grid(figure4.4b_plots,
                                  figure4.4b_leg,
                                  ncol = 1, rel_heights = c(1, 0.05))

figure4.4b_title <- ggdraw() + 
  draw_label("Study 4: Children, 4-5y (scored using adults' scales)", size = 16, fontface = 'bold', x = 0, hjust = 0)

figure4.4b_plots_leg_title <- plot_grid(figure4.4b_title, figure4.4b_plots_leg,
                                        ncol = 1, rel_heights = c(0.12, 1))
```

```{r, fig.width = 6, fig.asp = 0.4}
figure4.4b_plots_leg_title
```
...
```{r}
figure4.4c_plots <- plot_grid(plots_d4_46_scored_46_par[[1]] + 
                               theme(legend.position = "none"),
...
figure4.4c_leg <- get_legend(plots_d4_46_scored_46_par[[1]] +
                               theme(legend.position = "bottom", 
                                     legend.direction = "horizontal") +
                               scale_fill_manual("Target character", 
                                                 values = colors02,
...
figure4.4c_plots_leg <- plot_grid(figure4.4c_plots,
                                  figure4.4c_leg,
                                  ncol = 1, rel_heights = c(1, 0.05))

figure4.4c_title <- ggdraw() + 
  draw_label("Study 4: Children, 4-5y (scored using their own scales, 3-factor solution)", size = 16, fontface = 'bold', x = 0, hjust = 0)

figure4.4c_plots_leg_title <- plot_grid(figure4.4c_title, figure4.4c_plots_leg,
                                        ncol = 1, rel_heights = c(0.12, 1))
```

```{r, fig.width = 6, fig.asp = 0.4}
figure4.4c_plots_leg_title
```
...
```{r}
figure4.4d_plots <- plot_grid(plots_d4_46_scored_46_wdm[[1]] + 
                               theme(legend.position = "none"),
...
figure4.4d_leg <- get_legend(plots_d4_46_scored_46_wdm[[1]] +
                               theme(legend.position = "bottom", 
                                     legend.direction = "horizontal") +
                               scale_fill_manual("Target character", 
                                                 values = colors02,
...
figure4.4d_plots_leg <- plot_grid(figure4.4d_plots,
                                  figure4.4d_leg,
                                  ncol = 1, rel_heights = c(1, 0.05))

figure4.4d_title <- ggdraw() + 
  draw_label("Study 4: Children, 4-5y (scored using their own scales, 4-factor solution)", size = 16, fontface = 'bold', x = 0, hjust = 0)

figure4.4d_plots_leg_title <- plot_grid(figure4.4d_title, figure4.4d_plots_leg,
                                        ncol = 1, rel_heights = c(0.12, 1))
```

```{r, fig.width = 6, fig.asp = 0.7}
figure4.4d_plots_leg_title
```
...
```{r}
figure4.4 <- plot_grid(figure4.4a_plots_leg_title, figure4.4b_plots_leg_title,
                       figure4.4c_plots_leg_title, figure4.4d_plots_leg_title, 
                       rel_heights = c(1, 1, 1, 2), ncol = 1)

figure4.4_cap <- add_sub(figure4.4, str_wrap("Figure 4.4: Relationships among US adults' and 4- to 5-year-old children's attributions of conceptual units in Study 4, organized by sample and scales used (rows) and by pair of conceptual units (columns). For children, three sets of scores are presented: using adults' scales (B1-B3), using younger children's own scales as indexed by the three-factor EFA solution (C1-C3), and using younger children's own scales as indexed by the four-factor EFA solution (D1-D6). See Table 4.4 for all scales. For each conceptual unit, scores could range from 0-1. Individual participants are plotted as small, translucent circles, and mean scores by character are plotted as larger, solid diamonds. Error bars are 95% bootstrapped confidence intervals. The dotted line corresponds to equal endorsements of the two conceptual units plotted.", 110), x = 0, hjust = 0)
```

```{r, include = T, fig.width = 5, fig.asp = 2.2}
