...
regtabs_all_ad <- bind_rows(regtab_d1a_ad_scored_ad_diff,
  ...
  mutate(age_group = "Adults",
         CI95 = gsub("\\[", "", CI95),
         CI95 = gsub("\\]", "", CI95)) %>%
  separate(CI95, c("ci_lower", "ci_upper"), sep = ", ", remove = F, convert = T)
```
...
```{r}
df_annot <- data.frame(pair = levels(factor(regtabs_all$pair)),
...
figure4.10 <- ggplot(regtabs_all %>% 
  ...
  geom_text(aes(label = nonzero, y = star_pos),
  ...
  geom_text(data = df_annot, show.legend = F,
            aes(x = NULL, y = NULL, group = NULL, color = NULL, shape = NULL,
                label = pos), x = 2, y = 0.7, hjust = 0.5, vjust = 1, size = 3) +
  geom_text(data = df_annot, show.legend = F,
            aes(x = NULL, y = NULL, group = NULL, color = NULL, shape = NULL,
  ...
  scale_y_continuous("Parameter estimate (b)", 
                     limits = c(-0.74, 0.74), breaks = seq(-1, 1, 0.2)) +
  labs(# title = "Intercepts from regression analyses of difference scores",
       # subtitle = "Error bars are 95% CIs from independent Bayesian regressions for each pair of conceptual units and each sample of participants,\naccounting for differences between characters and random intercepts for participants when appropriate (Studies 1d and 2).\nAsterisks indicate CIs that do not include zero.",
       x = "Age group") +
...
```{r}
figure4.10_cap <- add_sub(figure4.10, str_wrap("Figure 4.10: Intercepts from regression analyses of difference scores. For each pair of conceptual units, the intercept provides an index of the asymmetry in participants' attributions of these two conceptual units: If the intercept is positive participants in this sample tended to attribute the first unit more strongly than the second, and if the intercept is negative participants in this sample tended to attribute the second unit more strongly than the first. Error bars are 95% CIs from independent Bayesian regressions for each pair of conceptual units, for each sample of participants, accounting for differences between charactesr and including random intercepts for particpipants when appropriate (Studies 1d and 2). Asterisks indicate CIs that do not include zero.", 105), x = 0, hjust = 0)
```

```{r, include = T, fig.width = 5, fig.asp = 0.6}
...
XX __INSERT GENERAL DISCUSSION__
