...
- for any sample, I have identified relevant conceptual units (chapter II)
- now, I want to "score" each participant in that sample on the degree to which they endorsed that unit for that target character. in other words, not just did they attribute particular mental capacities, but did they attribute that unit (and to what degree).
- to do this, exploited EFAs from chapter II to a new end (and this is often what people use EFA for): constructing a "scale" for each conceptual unit (for each sample, for each study). details:
    - sort capacities by "dominant" factor, considering only positive loadings
    - skim off capacities with the lowest factor loading until we have scales of equal length for all factors
    - "score" participants by averaging their responses across the mental capacities associated with each factor (i.e., like a normal "scale")
- lots of alternatives to this (e.g., could consider absolute rather than only positive loadings, could skip the part where I make the scales equal length; could use only the top 3, 4, 5, ... mental capacities for each factor, could throw out mental capacities that cross-load beyond a certain threshhold on other non-dominant factors, could throw out mental capacities that don't meet a certain threshhold of strong factor loadings on dominant factor), but i think this maximizes comparability across samples (especially across age groups)
- could also look at factor scores, but they are like z-scores, so they don't provide a sense of absolute minimum/maximum score (key to interpretation goals)
