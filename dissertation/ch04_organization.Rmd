...
```{r}
figure4.2all_plots <- plot_grid(plots_agegp_d2_scored_ad[[1]] + 
                               theme(legend.position = "none"),
...
figure4.2all_leg <- get_legend(plots_agegp_d2_scored_ad[[1]] +
                               theme(legend.position = "bottom", 
                                     legend.direction = "horizontal") +
                               scale_color_manual("Target character",
                                                  values = colors02,
...
figure4.2all_plots_leg <- plot_grid(figure4.2all_plots,
                                  figure4.2all_leg,
                                  ncol = 1, rel_heights = c(1, 0.05))

figure4.2all_title <- ggdraw() + 
  draw_label("Tracking development between 7-9y and adulthood (scored using adults' scales)", 
             size = 16, fontface = 'bold', x = 0, hjust = 0)

figure4.2all_plots_leg_title <- plot_grid(figure4.2all_title, figure4.2all_plots_leg,
                                        ncol = 1, rel_heights = c(0.12, 1))
```

```{r, fig.width = 6, fig.asp = 0.4}
figure4.2all_plots_leg_title
```
...
```{r}
figure4.3all_plots <- plot_grid(plots_agegp_d3_scored_ad[[1]] + 
                               theme(legend.position = "none"),
...
figure4.3all_leg <- get_legend(plots_agegp_d3_scored_ad[[1]] +
                               theme(legend.position = "bottom", 
                                     legend.direction = "horizontal") +
                               scale_color_manual("Target character",
                                                  values = colors09,
...
figure4.3all_plots_leg <- plot_grid(figure4.3all_plots,
                                  figure4.3all_leg,
                                  ncol = 1, rel_heights = c(1, 0.05))

figure4.3all_title <- ggdraw() + 
  draw_label("Tracking development between 4-9y and adulthood (scored using adults' scales)", 
             size = 16, fontface = 'bold', x = 0, hjust = 0)

figure4.3all_plots_leg_title <- plot_grid(figure4.3all_title, figure4.3all_plots_leg,
                                        ncol = 1, rel_heights = c(0.12, 1))
```

```{r, fig.width = 6, fig.asp = 0.4}
figure4.3all_plots_leg_title
```

```{r}
figure4.4 <- plot_grid(figure4.3a_plots_leg_title, figure4.3b_plots_leg_title,
                       figure4.3d_plots_leg_title, figure4.3all_plots_leg_title,
                       ncol = 1)
...
```{r, include = T, fig.width = 5, fig.asp = 1.85}
...
```{r}
figure4.5all_plots <- plot_grid(plots_agegp_d4_scored_ad[[1]] + 
                               theme(legend.position = "none"),
...
figure4.5all_leg <- get_legend(plots_agegp_d4_scored_ad[[1]] +
                               theme(legend.position = "bottom", 
                                     legend.direction = "horizontal") +
                               scale_color_manual("Target character",
                                                  values = colors02,
...
figure4.5all_plots_leg <- plot_grid(figure4.5all_plots,
                                  figure4.5all_leg,
                                  ncol = 1, rel_heights = c(1, 0.05))

figure4.5all_title <- ggdraw() + 
  draw_label("Tracking development between 4-5y and adulthood (scored using adults' scales)", 
             size = 16, fontface = 'bold', x = 0, hjust = 0)

figure4.5all_plots_leg_title <- plot_grid(figure4.5all_title, figure4.5all_plots_leg,
                                        ncol = 1, rel_heights = c(0.12, 1))
```

```{r, fig.width = 6, fig.asp = 0.4}
figure4.5all_plots_leg_title
```
...
figure4.7_cap <- add_sub(figure4.7, str_wrap("Figure 4.7: Relationships among US adults', older children's, and younger children's attributions of conceptual units in Study 3, scored using their own scales scales (see Table 4.3). Ttwo sets of scores are presented: using a three-factor EFA solution (A1-A3) and using a four-factor EFA solution (B1-B6). Plots are organized by scale used (rows) and by pair of conceptual units (columns). For each conceptual unit, scores could range from 0-1. Individual participants are plotted as small, translucent circles, and mean scores by character are plotted as larger, solid diamonds. Error bars are 95% bootstrapped confidence intervals. The dotted line corresponds to equal endorsements of the two conceptual units plotted.", 110), x = 0, hjust = 0)
```
