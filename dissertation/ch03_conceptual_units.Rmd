...
## Total variance explained: How "big" is the set of conceptual units identified?
...
The total variance explained by each of the EFA solutions discussed in this chapter is illustrated in Figure 3.5, panel C.

__BOOKMARK ADD DISCUSSION__.

## Proportion of variance explained by each factor: How "big" is a particular conceptual unit?

A researcher might also be interested in assess the size of a single conceptual unit, either in the absolute or in relation to the other conceptual units identified by that EFA solution. 

The _proportion of total variance_ in all the measured variables explained by a particular factor can be calculated by diving the sum of squared loadings for that factor across all variables by the total number of measured variables. (The _total variance_ for a given EFA solution, discussed in the previous section, is the sum of these proportions across all factors in that solution.) This could range, in theory, from 0% up to the total variance explained by all factors in that solution, and provides an indication of the absolute size of the particualr conceptual unit in question. The proportion of total variance explained by each factor, for each EFA solution discussed in this chapter, is illustrated in Figure 3.5, panel B.
...
In my view, this is the most useful way to estimate and compare the size of conceptual units across the various studies and age groups presented in this chapter, because it allows me to compare the sizes of similar conceptual units (e.g., factors that I have labeled BODY) that I have identified in different age groups or in studies using different experimental paradigms, even though these age groups or studies might vary in the _total variance_ explained by their respective EFA solutions (which would, in turn, impose different constraints on how much of this total variance each factor could explain, in theory). For example, in Study 4, the three-factor solution for adults explained `r var_total_sum %>% filter(study == "Study 4", sample == "adults", solution == "3-factor") %>% select(value) %>% as.numeric() %>% round(2)*100`% of the total variance, which places a relatively high "ceiling" on the proportion of total variance coudl be explained by a single factor; in comparison, the three-factor solution for children explained `r var_total_sum %>% filter(study == "Study 4", sample == "4-5y", solution == "3-factor") %>% select(value) %>% as.numeric() %>% round(2)*100`% of the total variance, placing a much lower "ceiling" on the proportion of total variance could be explained by a single factor. While it is interesting to note that the BODY-like factors in these solutions explained `r var_total_report %>% filter(study == "Study 4", sample == "adults", solution == "3-factor", ur_factor == "BODY") %>% select(value) %>% as.numeric() %>% round(2)*100`% of the total variance in adults' responses and only `r var_total_report %>% filter(study == "Study 4", sample == "4-5y", solution == "3-factor", ur_factor == "BODY") %>% select(value) %>% as.numeric() %>% round(2)*100`% of the total variance in children's responses (Figure 3.5, panel B), it is in my view more illuminating that in both solutions the BODY-like factor accounting for a similar proportion of the _shared_ variance (among adults: `var_shared_report %>% filter(study == "Study 4", sample == "adults", solution == "3-factor", ur_factor == "BODY") %>% select(value) %>% as.numeric() %>% round(2)*100`%; among children: `var_shared_report %>% filter(study == "Study 4", sample == "4-5y", solution == "3-factor", ur_factor == "BODY") %>% select(value) %>% as.numeric() %>% round(2)*100`%).

while the three-factor solution for The proportion of shared variance explained by each factor, for each EFA solution discussed in this chapter, is illustrated in Figure 3.5, panel A.

Among adults (Figure 3.5, panels A and B, left column), a clear pattern emerged in the relative size of these factors as indexed by the proportion of variance explained (shared or total): In all eight of the EFA solutions included in this chapter, adults' BODY and HEART factors explained a disproportionately large amount of the variance (more than would be expected if all factors were equal in size; see panel A), and their MIND factor explained a disproportionately small amount (less than would be expected if al factors were equal in size). In most of these solutions, the BODY factor explained slightly more variance than the HEART factor, but these differences were generally quite small. This suggests that the conceptual units I have referred to as BODY and HEART may play especially large roles in US adults' representations of mental life, at least in the way they use these representations to distinguish among different kinds of beings in the world.

Among 7- to 9-year-old children (Figure 3.5, panels A and B, middle column), a similar pattern was observed in Study 3 (orange), with children's BODY and HEART factors explaining more variance than their MIND factor. However, in Study 2 (purple), children's HEART factor explained nearly twice as much of the variance as either of the other factors. This raises the possibility that the conceptual unit I have called HEART looms especially large in the representations of children in this age range—perhaps because it has emerged relatively recently as a distinct unit in its own right. Further studies would be required to determine whether this phenomenon is reliable and the circumstances under which is most or less likely to manifest.

Among 4- to 6-year-old children (Study 3) and 4- to 5-year-old children (Study 4; Figure 3.5, panels A and B, right column), the BODY-like factors explained disproportionately large amounts of variance. This holds true across all solutions for both studies (when combining the two BODY-like factors in the four-factor solution) and was particularly pronounced in the two-factor solution for Study 3 (orange, top row), in which younger children's BODY-HEART factor was nearly twice the 'size' of their MIND factor—perhaps a harbinger of an impending split between BODY and HEART as young children's conceptual representations become more like the older children and adults around them. The variance explained by younger children's HEART-like factors in the three-factor solutions for Studies 3 and 4 (middle row) appears to have been somewhat smaller than it was among older children and adults, particularly in Study 4 (turquoise), while the variance explained by the more MIND-like factors appears to have been roughly comparable to that of adults in all studies. 
...
__BOOKMARK: ADD DISCUSSION OF TOTAL VARIANCE EXPLAINED__
