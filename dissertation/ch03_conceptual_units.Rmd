...
In analyzing these datasets with an eye toward identifying "conceptual units," the basic insight is that tracking the covariance of mental capacity attributions provides a way of identifying suites of mental capacities that "hang together" in reasoning about mental life, and that these suites of mental capacities might correspond to the units of some larger conceptual representation of this general domain. In Chapter II, I illustrated this with the following example: If participants who endorse Capacity X also tend to endorse Capacities Y and Z, this provides some evidence that Capacities X, Y, and Z consitute a suite of mental capacities that are closely associated with the same underlying "conceptual unit." Here I will translate this general intuition into a specific analysis plan to be applied to each of these datasets in turn. 

One way of restating the general intuition just described would be to say that the goal of this analysis was to uncover a set of latent constructs that might plausibly have given rise to the observed correlations among mental capacity attributions in each group of participants. A canonical way to identify latent constructs through observed correlations is exploratory factor analysis (EFA), a form of dimensionality reduction that posits that the observed variables in a given dataset are related, to varying degrees, to a smaller set of latent "factors"; and that individual observations of each of these variables are considered to reflect a combination of the state of these latent factors, that particular variable's relationship to each of these latent factors, and noise. Following this logic, I posit that, for any of the current datasets, the various mental capacities included in that study are related, to varying degrees, to a smaller set of latent "factors," which I will refer to as "conceptual units"; and that a participant's attributions of each fo these mental capacities to a particular target character reflects a combination of that participant's beliefs about the extent to which these conceptual units apply to that target character, that particular mental capacity's relationships to each of these conceptual units, and noise.
