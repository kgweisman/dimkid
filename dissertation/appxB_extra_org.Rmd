...
```{r}
plots_d4_46_scored_46_par <- relviz_fun(d4_46_scored_46_par, colors = colors09)
```
...
fig_d4_46_scored_46_par_plots_leg <- plot_grid(fig_d4_46_scored_46_par_plots, fig_d4_46_scored_46_par_leg,
                                               ncol = 1, rel_heights = c(1, 0.05))

fig_d4_46_scored_46_par_title <- ggdraw() + 
  draw_label("Study 4: Children, 4-6y (using their own scales, 2-factor solution)", size = 16, fontface = 'bold', x = 0, hjust = 0)
...
```{r}
# r_d4_46_scored_46_par_diff_BODYHEART_MINDHEART <- brm(
#   diff ~ 1 + character,
#   data = d4_46_scored_46_par_diff %>% filter(pair == "BODY-HEART - MIND-HEART"),
#   cores = 4)
# 
# saveRDS(r_d4_46_scored_46_par_diff_BODYHEART_MINDHEART,
#         "./stored/brms_models/r_d4_46_scored_46_par_diff_BODYHEART_MINDHEART")

r_d4_46_scored_46_par_diff_BODYHEART_MINDHEART <- readRDS("./stored/brms_models/r_d4_46_scored_46_par_diff_BODYHEART_MINDHEART")

summary(r_d4_46_scored_46_par_diff_BODYHEART_MINDHEART)
```

```{r}
regtab_d4_46_scored_46_par_diff <- diff_reg_table_fun(
  reg_list = list(r_d4_46_scored_46_par_diff_BODYHEART_MINDHEART),
  pair_list = "BODY-HEART - MIND-HEART",
  study_name = "Children, 4-6y (using their own scales, 2-factor solution)",
...
Visualizations of relationships among 4- to 6-year-old children's scores on the _BODY-HEART_ and _MIND-HEART_ scales are provided in Figure B.xx, row B. The results of a Bayesian regression comparing difference scores between conceptual units to zero, controlling for differences in assessments of the nine "diverse characters" that were featured as target characters in these studies, are provided in Table B.xx.

The main take-away of this analysis is that 4- to 6-year-old children reliably endorsed _BODY-HEART_ items more strongly than _MIND-HEART_ items.
...
The main take-aways of these analyses are that, among 7- to 9-year-old children in Study 2, _BODY_ vs. _HEART_ difference scores did not vary systematically with age, but both _BODY_ vs. _MIND_ and _HEART_ vs. _MIND_ difference scores became increasingly adult-like with age. See "Exact age (centered)" rows in Table B.3, and see Figure B.5, row A, for a visualization of difference scores over age.
...
The main take-aways of these analyses are that, among 4- to 9-year-old children in Study 3, all three difference scores (_BODY_ vs. _HEART_, _BODY_ vs. _MIND_, and _HEART_ vs. _MIND_) became increasingly adult-like with age. See "Exact age (centered)" rows in Table B.B, and see Figure B.5, row A, for a visualization of difference scores over age.
