...
newfun <- function(df, which_efa, colors, fun = "diff", points = T) {
  ...
  plot_df <- plot_df %>%
    select(-c(F1, F2, F3)) %>%
    gather(factors, comp, starts_with("F")) %>%
    distinct()
...
```{r}
newfun_stat <- function(df, which_efa, colors, fun = "diff") {
  ...
  if(fun == "ratio"){
    results <- lme4::lmer(I(comp - 1) ~ 0 + factors + (1 | subid), plot_df)
  }
...
```{r}
newfun(df = rename_fun(d1), which_efa = efa_pnasd1, 
       colors = c("#fb9a99", "#1f78b4"), fun = "ratio", points = F)
newfun(df = rename_fun(d2), which_efa = efa_pnasd2,
       colors = c("#fb9a99", "#1f78b4"), fun = "ratio", points = F)
newfun(df = d3 %>%
         rename_fun() %>%
         select(-condition) %>%
       ...
       which_efa = efa_pnasd3, colors = c("#fb9a99", "#1f78b4"), 
       fun = "ratio", points = F)
newfun(df = rename_fun(d4), which_efa = efa_pnasd4,
       colors = colors21, fun = "ratio", points = F)
```
