...
```{r}
IFcor_study1 <- bind_rows(
  # 3-factor solutions
  IFcor_fun(efa_3_d1a_ad_oblimin, factor_names_efa_3_d1a_ad_oblimin) %>%
    mutate(study = "Study 1a", solution = "3-factor solution"),
  IFcor_fun(efa_3_d1b_ad_oblimin, factor_names_efa_3_d1b_ad_oblimin) %>%
    mutate(study = "Study 1b", solution = "3-factor solution"),
  IFcor_fun(efa_3_d1c_ad_oblimin, factor_names_efa_3_d1c_ad_oblimin) %>%
    mutate(study = "Study 1c", solution = "3-factor solution"),
  IFcor_fun(efa_3_d1d_ad_oblimin, factor_names_efa_3_d1d_ad_oblimin) %>%
    mutate(study = "Study 1d", solution = "3-factor solution"),

  # 4-factor solutions
  IFcor_fun(efa_4_d1a_ad_oblimin, factor_names_efa_4_d1a_ad_oblimin) %>%
    mutate(study = "Study 1a", solution = "4-factor solution"),
  IFcor_fun(efa_4_d1b_ad_oblimin, factor_names_efa_4_d1b_ad_oblimin) %>%
    mutate(study = "Study 1b", solution = "4-factor solution"),
  IFcor_fun(efa_4_d1c_ad_oblimin, factor_names_efa_4_d1c_ad_oblimin) %>%
    mutate(study = "Study 1c", solution = "4-factor solution"),
  IFcor_fun(efa_4_d1d_ad_oblimin, factor_names_efa_4_d1d_ad_oblimin) %>%
    mutate(study = "Study 1d", solution = "4-factor solution"),

  # 5-factor solutions
  IFcor_fun(efa_5_d1a_ad_oblimin, factor_names_efa_5_d1a_ad_oblimin) %>%
    mutate(study = "Study 1a", solution = "5-factor solution"),
  IFcor_fun(efa_5_d1b_ad_oblimin, factor_names_efa_5_d1b_ad_oblimin) %>%
    mutate(study = "Study 1b", solution = "5-factor solution"),
  IFcor_fun(efa_5_d1c_ad_oblimin, factor_names_efa_5_d1c_ad_oblimin) %>%
    mutate(study = "Study 1c", solution = "5-factor solution"),
  IFcor_fun(efa_5_d1d_ad_oblimin, factor_names_efa_5_d1d_ad_oblimin) %>%
    mutate(study = "Study 1d", solution = "5-factor solution")) %>%
  
  # all studies
  filter(factor1 != factor2) %>%
  distinct() %>%
  mutate(ur_factor1 = case_when(factor1 == "BODY-HEART" ~ "BODY-HEART",
                                grepl("BODY", factor1) ~ "BODY",
                                grepl("HEART", factor1) ~ "HEART",
                                grepl("MIND", factor1) ~ "MIND"),
         ur_factor1 = factor(ur_factor1,
                             levels = c("BODY", "BODY-HEART", 
                                        "HEART", "MIND")),
         ur_factor1_pos = recode(ur_factor1,
                                 "BODY" = 1, "BODY-HEART" = 1.5,
                                 "HEART" = 2, "MIND" = 3)) %>%
  mutate(ur_factor2 = case_when(factor2 == "BODY-HEART" ~ "BODY-HEART",
                                grepl("BODY", factor2) ~ "BODY",
                                grepl("HEART", factor2) ~ "HEART",
                                grepl("MIND", factor2) ~ "MIND"),
         ur_factor2 = factor(ur_factor2,
                             levels = c("BODY", "BODY-HEART", 
                                        "HEART", "MIND")),
         ur_factor2_pos = recode(ur_factor2,
                                 "BODY" = 1, "BODY-HEART" = 1.5,
                                 "HEART" = 2, "MIND" = 3)) %>%
  select(study, solution, ur_factor1_pos, ur_factor1, factor1, 
         ur_factor2_pos, ur_factor2, factor2, cor) %>%
  arrange(study, solution, ur_factor1_pos, ur_factor2_pos)
...
IFcor_study1 %>%
  mutate(ur_pair = interaction(ur_factor1, ur_factor2),
         ur_pair = case_when(
           ur_pair %in% c("BODY.HEART", "HEART.BODY") ~ "BODY.HEART",
           ur_pair %in% c("BODY.MIND", "MIND.BODY") ~ "BODY.MIND",
           ur_pair %in% c("HEART.MIND", "MIND.HEART") ~ "HEART.MIND")) %>%
  mutate(lab = case_when(
    factor1 %in% c("BODY", "HEART", "MIND") & 
      factor2 %in% c("BODY", "HEART", "MIND") ~ " ",
    TRUE ~ paste(factor1, factor2, sep = " vs. "))) %>%
  arrange(study, solution, ur_pair) %>%
  ggplot(aes(x = ur_pair, y = cor, color = study)) +
  ...
  # geom_text(aes(label = lab), show.legend = F,
  #           position = pos, hjust = 0) +
  ...
  labs(title = "Inter-factor correlations across studies, age groups, and EFA solutions",
       x = "Factors", y = "Correlation", color = "Study") +
...
```{r}
# adults
plot_efa_3_d2_ad_oblimin <- heatmap_fun(efa_3_d2_ad_oblimin) +
  labs(title = "Adults", subtitle = "Oblimin-transformed 3-factor solution") +
  theme(legend.position = "none")

plot_efa_4_d2_ad_oblimin <- heatmap_fun(efa_4_d2_ad_oblimin) +
  labs(title = "Adults", subtitle = "Oblimin-transformed 4-factor solution") +
  theme(legend.position = "none")

# children
plot_efa_3_d2_79_oblimin <- heatmap_fun(efa_3_d2_79_oblimin) +
  labs(title = "Chidlren, 7-9y", subtitle = "Oblimin-transformed 3-factor solution") # +
  # theme(legend.position = "none")
```

```{r}
figureA.6 <- plot_grid(plot_efa_3_d2_ad_oblimin, plot_efa_4_d2_ad_oblimin, 
                       plot_efa_3_d2_79_oblimin, 
                       labels = "AUTO", ncol = 3, rel_widths = c(1, 1.2, 1.2))

figureA.6_cap <- add_sub(figureA.6, str_wrap("Figure A.6: Factor loadings from exploratory factor analyses of Study 2 (as reported in Chapter III), after oblimin transformation. (A) Results for US adults, retaining three factors. (B) Results for US adults, retaining four factors. (C) Results for US children ages 7-9y. A factor loading of +1 indicates a perfectly positive relationship between mental capacity and underlying construct; a loading of -1 indicates a perfectly negative relationship.", 210), x = 0, hjust = 0)
```

```{r figureA.6, include = T, fig.width = 9, fig.asp = 0.6}
...
```{r}
# adults
plot_efa_3_d3_ad_oblimin <- heatmap_fun(efa_3_d3_ad_oblimin) +
  labs(title = "Adults", subtitle = "Oblimin-transformed 3-factor solution") +
  theme(legend.position = "none")

# older children
plot_efa_3_d3_79_oblimin <- heatmap_fun(efa_3_d3_79_oblimin) +
  labs(title = "Older children, 7-9y", subtitle = "Oblimin-transformed 3-factor solution") +
  theme(legend.position = "none")

# younger children
plot_efa_3_d3_46_oblimin <- heatmap_fun(efa_3_d3_46_oblimin) +
  labs(title = "Younger children, 4-6y", subtitle = "Oblimin-transformed 3-factor solution") +
  theme(legend.position = "none")

plot_efa_2_d3_46_oblimin <- heatmap_fun(efa_2_d3_46_oblimin) +
  labs(title = "Younger children, 4-6y", subtitle = "Oblimin-transformed 2-factor solution") +
  theme(legend.position = "none")
```

```{r}
figureA.7 <- plot_grid(plot_efa_3_d3_ad_oblimin, plot_efa_3_d3_79_oblimin,
                       plot_efa_3_d3_46_oblimin, plot_efa_2_d3_46_oblimin,
                       labels = "AUTO", ncol = 4, rel_widths = c(1, 1, 1, 1))

figureA.7_cap <- add_sub(figureA.7, str_wrap("Figure A.7: Factor loadings from exploratory factor analyses of Study 3 (as reported in Chapter III), after oblimin transformation. (A) Results for US adults. (B) Results for US children ages 7-9y. (C) Results for US children ages 4-6y, retaining three factors. (D) Results for US children ages 4-6y, retaining two factors. A factor loading of +1 indicates a perfectly positive relationship between mental capacity and underlying construct; a loading of -1 indicates a perfectly negative relationship.", 210), x = 0, hjust = 0)
```
...
```{r}
# adults
plot_efa_3_d4_ad_oblimin <- heatmap_fun(efa_3_d4_ad_oblimin) +
  labs(title = "Adults", subtitle = "Oblimin-transformed 3-factor solution") +
  theme(legend.position = "none")

# younger children
plot_efa_3_d4_46_oblimin <- heatmap_fun(efa_3_d4_46_oblimin) +
  labs(title = "Children, 4-5y", subtitle = "Oblimin-transformed 3-factor solution") +
  theme(legend.position = "none")

plot_efa_4_d4_46_oblimin <- heatmap_fun(efa_4_d4_46_oblimin) +
  labs(title = "Children, 4-5y", subtitle = "Oblimin-transformed 4-factor solution") +
  theme(legend.position = "none")
```

```{r}
figureA.8 <- plot_grid(plot_efa_3_d4_ad_oblimin, plot_efa_3_d4_46_oblimin, 
                       plot_efa_4_d4_46_oblimin,
                       labels = "AUTO", ncol = 3, rel_widths = c(1, 1, 1.4))

figureA.8_cap <- add_sub(figureA.8, str_wrap("Figure A.8: Factor loadings from exploratory factor analyses of Study 4 (as reported in Chapter III), after oblimin transformation. (A) Results for US adults. (B) Results for US children ages 4-5y, retaining three factors. (C) Results for US children ages 4-5y, retaining four factors. A factor loading of +1 indicates a perfectly positive relationship between mental capacity and underlying construct; a loading of -1 indicates a perfectly negative relationship.", 210), x = 0, hjust = 0)
```

```{r figureA.8, include = T, fig.width = 9, fig.asp = 0.6}
...
The factor retention criteria employed in Weisman et al. (2017) suggested retaining 3 factors for all of the studies included in Study 1; see Chapter III. In contrast, parallel analysis suggested retaining 4 factors (instead of 3) for Studies 1b and 1d; minimizing BIC suggested retaining 4 factors (instead of 3) for Study 1c, and 5 factors (instead of 3) for Studies 1a, 1b, and 1d. See Figure A.9.
...
The factor retention criteria employed in Weisman et al. (2017) suggested retaining 3 factors for all of the age groups included in Study 3; see Chapter III. In contrast, parallel analysis suggested retaining 2 factors (instead of 3) for younger children; and minimizing BIC suggested retaining 4 factors (instead of 3) for adults, and 1 factor (i.e., a "null" solution) for younger children (ages 4-6y). See Figure A.10.
...
```{r}
figureA.10 <- plot_grid(plot_efa_bic_d3_ad, plot_efa_bic_d3_46,
                       labels = "AUTO", ncol = 2, rel_widths = c(1, 1))

figureA.10_cap <- add_sub(figureA.10, str_wrap("Figure A.10: Factor loadings from additional exploratory factor analyses of Study 3, not reported in Chapter III. See Chapter II for methods. (A) Adults, solution suggested by minimizing BIC. (B) Younger children (4-6y), 'null' solution suggested by minimizing BIC. In this and all figures portraying factor loadings, factors have been plotted in the same order (BODY, HEART, MIND), when applicable, to facilitate comparison across studies. A factor loading of +1 indicates a perfectly positive relationship between mental capacity and underlying construct; a loading of -1 indicates a perfectly negative relationship.", 115), x = 0, hjust = 0)
```

```{r figureA.10, include = T, fig.width = 5, fig.asp = 1}
ggdraw(figureA.10_cap)
```

## Study 4

All of the EFA solutions suggested by the three factor retention protocols are reported in full in Chapter III, with the exception of the 1-factor ("null") solution suggested by minimizing BIC for younger children (4-5y). See Figure A.11.
